...
class Encoder(nn.Module):
    def __init__(self, features_dim, d_in_hidden, d_model, d_ff, d_k, d_v, n_layers, n_heads, d_out_hidden, d_tower_hidden):
        ...
        self.outputs = nn.Sequential(nn.Linear(d_model, d_out_hidden), nn.ReLU(), nn.Linear(d_out_hidden, d_tower_hidden))
...
class MTLnet(nn.Module):
    def __init__(self, features_dim, d_in_hidden, d_model, d_ff, d_k, d_v, n_layers, n_heads, d_out_hidden, d_tower_hidden, d_tower_hidden2):
        super(MTLnet, self).__init__()
        self.sharedlayer = Encoder(features_dim, d_in_hidden, d_model, d_ff, d_k, d_v, n_layers, n_heads, d_out_hidden, d_tower_hidden)
        self.tower1 = nn.Sequential(nn.Linear(d_tower_hidden, d_tower_hidden2), nn.ReLU(), nn.Linear(d_tower_hidden2, 1))
        self.tower2 = nn.Sequential(nn.Linear(d_tower_hidden, d_tower_hidden2), nn.ReLU(), nn.Linear(d_tower_hidden2, 1))

    def forward(self, x, mask, fix_len):
        h_shared, att = self.sharedlayer(x, mask, fix_len) # [batch_size, src_len, d_model]
        out1 = self.tower1(h_shared)
        out2 = self.tower2(h_shared)
        return out1, out2  # out1为IDR out2为DFL